...
public interface IProyectoRepository extends JpaRepository<Proyecto, Integer> {
    ...
    @Query("SELECT new pe.edu.upc.si62_grupo_01.dtos.ProyectoCountDTO(p.usuario.nombreCompleto, COUNT(p)) " +
            "FROM Proyecto p WHERE p.usuario.idUsuario = :idUsuario GROUP BY p.usuario.nombreCompleto")
